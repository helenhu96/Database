...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		while (it.hasNext() && key.compare(Op.LESS_THAN, f)) {
			entry = it.next();
			key = entry.getKey();
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		Tuple t = null;
		while (newpg.getNumTuples() < pgNum/2) {
			t = it.next();
			page.deleteTuple(t);
			newpg.insertTuple(t);
		}
		Field key = t.getField(this.keyField());
		BTreeEntry entry = new BTreeEntry(key, pgid, newpg.getId());
		BTreeInternalPage parent = this.getParentWithEmptySlots(tid, dirtypages, parentid, field);
		parent.insertEntry(entry);
		parent.updateEntry(entry);
		BTreePageId right = null;
		BTreePageId newid = newpg.getId();
		if (page.getRightSiblingId() != null) {
			right = page.getRightSiblingId();
			BTreeLeafPage rightpg = (BTreeLeafPage) this.getPage(tid, dirtypages, right, Permissions.READ_WRITE);
			rightpg.setLeftSiblingId(newid);
		}
		page.setRightSiblingId(newid);
		newpg.setLeftSiblingId(pgid);
		newpg.setRightSiblingId(right);
		newpg.setParentId(page.getParentId());
		return key.compare(Op.LESS_THAN, field) ? newpg : page; 
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		entry.setRightChild(newpg.getId());
		parent.insertEntry(new BTreeEntry(f, page.getId(), newpg.getId()));		
	
		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, newpg);
		return field.compare(Op.LESS_THAN_OR_EQ, f) ? page : newpg;
	}
	 ...
	 * @return the page, guaranteed to have at least one empty slot
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		Tuple insertTup = right.iterator().next();
		Field key = insertTup.getField(this.keyField());
		entry.setKey(key);
		parent.updateEntry(entry);
	}
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		while (page.getNumEntries() < half) {
			...
			next.setKey(pkey);
			Iterator<BTreeEntry> right = page.iterator();
			next.setLeftChild(next.getRightChild());
			next.setRightChild(right.next().getLeftChild());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		int half = (page.getNumEntries() + rightSibling.getNumEntries())/2;
		...
		while (page.getNumEntries() < half) {
			...
			parent.updateEntry(parentEntry);
			next.setKey(key);
			Iterator<BTreeEntry> left = page.reverseIterator();
			next.setRightChild(next.getLeftChild());
			next.setLeftChild(left.next().getRightChild());
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreePageId rightid = null;
		if (rightPage.getRightSiblingId()!=null) {
			rightid = rightPage.getRightSiblingId();
			BTreeLeafPage right = (BTreeLeafPage) this.getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_ONLY);
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry insert = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild() ,rightPage.iterator().next().getLeftChild());
		leftPage.insertEntry(insert);
		while (rightPage.getNumEntries() > 0) {
			BTreeEntry rightnext = right.next();
			rightPage.deleteKeyAndRightChild(rightnext);
			leftPage.insertEntry(rightnext);
		}
