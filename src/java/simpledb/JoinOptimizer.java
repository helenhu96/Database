...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp.equals(Predicate.Op.EQUALS)) {
	    		...
	    		} else if (t1pkey == false || t2pkey == false) {
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        //Replace the following
    		...
    		int length = joins.size();
    		for (int i=1; i<length+1; i++) {
    			Set<Set<LogicalJoinNode>> all = enumerateSubsets(joins, i);
    			for (Set<LogicalJoinNode> nodes: all) {
    				double bestcost = Double.MAX_VALUE;
    				CostCard curr = new CostCard();
    				curr.cost = Double.MAX_VALUE;
    				for (LogicalJoinNode node: nodes) {
    					CostCard costcard = computeCostAndCardOfSubplan(stats, filterSelectivities, node, nodes, bestcost, cache);
    					if (costcard!= null && costcard.cost < curr.cost) {
    						curr = costcard;
    					}
    				}
    				cache.addPlan(nodes, curr.cost, curr.card, curr.plan);
    			}
    		}
    		Set<LogicalJoinNode> set = new HashSet<>(joins);
    		Vector<LogicalJoinNode> best = cache.getOrder(set);
        return best;
    }
